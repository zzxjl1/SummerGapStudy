package com.idealbroker.aisee.opencv;
...
@TargetApi(21)
public class Camera2View extends CameraBridgeViewBase {

    private static final String LOGTAG = "Camera2View";
    ...
    protected boolean initializeCamera() {
        ...
        try {
            ...
            } else { // make Camera2View behaves in the same way as JavaCameraView
        ...
        try {
            ...
            mImageReader.setOnImageAvailableListener(new ImageReader.OnImageAvailableListener() {
                ...
                public void onImageAvailable(ImageReader reader) {
                    ...
                    Camera2Frame tempFrame = new Camera2Frame(image);
    ...
    private Size getOptimalSize(List<Size> size, int width, int height) {
        List<Size> sizeList = new ArrayList<>();
        for (Size option : size) {
            if (width > height) {
        ...
        if (sizeList.size() > 0) {
            return Collections.min(sizeList, new Comparator<Size>() {
                ...
                public int compare(Size lhs, Size rhs) {
                    return (int) (lhs.width * lhs.height - rhs.width * rhs.height);
                }
            });
        }
        Size result = sizeList.get(0);
        Log.e("PREVIEW", String.valueOf(result.width) + " " + String.valueOf(result.height));
        return result;
    }

    private static Size findClosestPreviewSizeValue(List<Size> supportedSize, int surfaceWidth, int surfaceHeight) {
        Log.e("PREVIEW supported", supportedSize.toString());
        // 如果x<y,交换x,y的值
        ...
        // 计算控件宽高比
        float referRatio = surfaceWidth * 1.0f / surfaceHeight;
        float lastDiff = Integer.MAX_VALUE;
        Size lastSize = null;
        for (Size size : supportedSize) {
            ...
            // 计算分辨率宽高比
            float supportRatio = (float) (size.width * 1.0f / size.height);
            // 计算差值的绝对值
    ...
    public Size calculateCameraFrameSize(List<Size> supportedSizes, int surfaceWidth, int surfaceHeight) {
        ...
        Log.e("PREVIEW supported", descSupportedSizes.toString());
        ...
        for (Size size : descSupportedSizes) {
            if (size.width >= targetWidth && size.height >= targetHeight) {
                Log.e("PREVIEW supported", String.valueOf(size.width) + " " + String.valueOf(size.height));
                calcWidth = (int) (size.width);
                calcHeight = (int) (size.height);
            }
        }
        Log.e("PREVIEW", String.valueOf(calcWidth) + " " + String.valueOf(calcHeight));
    ...
    protected Size calculateCameraFrameSize(List<?> supportedSizes, ListItemAccessor accessor, int surfaceWidth, int surfaceHeight) {
        ...
        return getOptimalSize(sizes, surfaceWidth, surfaceHeight);
        //return findClosestPreviewSizeValue(sizes, surfaceWidth, surfaceHeight);
        //return calculateCameraFrameSize(sizes, surfaceWidth, surfaceHeight);
    ...
    public class Camera2Frame implements CvCameraViewFrame {
        @Override
        public Mat gray() {
        ...
        public Camera2Frame(Image image) {
            super();
            mImage = image;
            mRgba = new Mat();
            mGray = new Mat();
